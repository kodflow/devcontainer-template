...
|--------|--------|--------|
| `dispatch.md` | exists | Phases 0–1.5: context, repo profile, intent/risk, auto-describe (draft) |
| `triage.md` | exists | Feedback/questions (draft), finding JSON schema, severity rubric, executor/specialist dispatch (TEAMS/SUBAGENTS) |
| `dimensions.md` | **author** | Full taxonomy (correctness, security, idioms, architecture, portability, perf, concurrency, error-handling, API/contract, testing, docs, deps/supply-chain, observability, data-privacy/PII, licensing, maintainability, a11y/i18n, doc & CLAUDE.md sync): macro+micro checks + owner agent per dimension |
| `deterministic.md` | **author** | Tier runner: per-language/IaC commands, `run()` helper, `_table.tsv` parsing, tool-status model |
| `graph.md` | **author** | Bounded call-graph/blast-radius + change-coupling (git diff hunk headers + ast-grep-if-present + rg; NEVER `sg`) |
| `manifest.md` | **author** | Manifest schema, file-class enum, micro_per_hunk, Judge protocol, and the `review-verify-manifest.sh` contract |
...
| `tiers.md` | exists | T2 Qodo (absent-skip) + T3 CodeRabbit (auth-probe) integration |
| `synthesis.md` | exists | Verdict (incl. INCONCLUSIVE), Needs-Verification gate, LOCAL report + /plan output |
| `cyclic.md` | exists | --loop correctness gate, patch-ID tracking, guardrails |
