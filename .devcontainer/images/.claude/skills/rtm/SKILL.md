---
name: rtm
description: 'Requirements Traceability Matrix. Tracks what the project promises and proves
  each promise is implemented and tested: CRUD over a features ledger, auto-learn from code
  changes, and a parallel audit reporting which features have code, which have tests, and
  which are claims only.'
allowed-tools:
- Read(**/*)
- Write(.claude/**)
- Edit(.claude/**)
...
when_to_use: Use to answer 'is this actually done' across a project's declared features, or
  to audit coverage before a release. To open an issue and a branch to work on something,
  use /feature or /fix.
---

# /rtm - Feature Tracking RTM (Requirements Traceability Matrix)
...
|-------|--------|-------------|
| 1-2 | Read `crud.md` | Init/migration + CRUD operations (add/edit/del/list/show) + hierarchy inference |
| 3 | Read `audit.md` | --checkup wave-based audit with auto-correction |
| Ref | Read `autolearn.md` | Journal actions, compaction, schema |
