...
- Verify `<workspace>/.githooks/commit-msg` exists AND is executable (defence-in-depth layer 2, issue #358). If missing or not +x: -20 points.
- Verify `git -C <workspace> config core.hooksPath` returns `.githooks`. If unset or different: -10 points (layer 2 wired but inactive).
...
- `+20` if `../_shared/team-mode.md` exists AND has ≥ 14 `^## ` sections
