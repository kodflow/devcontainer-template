...
# Seed .worktreeinclude so Claude Code worktrees inherit the local config.
#
# A worktree is a fresh checkout, so gitignored files are absent from it. The
# generated Claude config (.claude/settings.json, mcp.json) and .env are exactly
# that — which means a new worktree starts unguarded until the config is
# regenerated by hand. Claude Code copies any gitignored path listed here into
# every worktree it creates.
#
# Never overwrite: the file is project-owned, and a consumer may have added
# paths of their own.
step_worktree_include() {
    local workspace="${WORKSPACE_FOLDER:-/workspace}"
    local include_file="$workspace/.worktreeinclude"

    if [ -e "$include_file" ]; then
        log_info ".worktreeinclude already present — leaving it untouched"
        return 0
    fi

    cat > "$include_file" << 'WTEOF'
# Gitignored files carried into every worktree Claude Code creates.
# Uses .gitignore syntax; only paths that match AND are gitignored are copied,
# so tracked files are never duplicated.

# Generated Claude Code config — absent from a fresh worktree checkout.
...
WTEOF
    ...
    log_success "Seeded .worktreeinclude for Claude Code worktrees"
}
...
run_step "Worktree include"      step_worktree_include
