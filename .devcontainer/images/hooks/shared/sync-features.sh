...
export FEATURES_SYNC_FAST_FORWARDED=0
...
_sync_via_manifest() {
    ...
    if [ "$dst_hash" = "$prev_hash" ]; then
        # Consumer untouched since last image — safe overwrite (no-op cp).
        cp -a "$src" "$dst"
        return 0
    fi

    # Manifest schema v2: stale-but-clean fast-forward.
    # When the dst content matches ANY hash previously shipped for this path
    # (capped at PREV_HISTORY_CAP=8 generations in build-features-manifest.py),
    # the consumer is simply lagging behind the image build commit — no hand
    # edit. Overwrite silently and count under FEATURES_SYNC_FAST_FORWARDED so
    # postStart's summary line keeps visibility on the outcome. v1 manifests
    # have no previous_hashes field → query returns empty → fall through to
    # the divergent-fork branch.
    local prev_list
    prev_list=$(jq -r --arg r "$rel" '(.previous_hashes[$r] // []) | .[]' \
    ...
    if [ -n "$prev_list" ]; then
        ...
        while IFS= read -r h; do
            if [ "$h" = "$dst_hash" ]; then
                cp -a "$src" "$dst"
                FEATURES_SYNC_FAST_FORWARDED=$((FEATURES_SYNC_FAST_FORWARDED + 1))
                return 0
            fi
        done <<< "$prev_list"
    fi

    log_warning "Preserving features/${rel}: divergent from upstream and from all known shipped versions (run 'git diff -- .devcontainer/features/${rel}' to inspect; delete file to restore template)"
    return 2
}
...
sync_features_tree() {
    ...
    FEATURES_SYNC_FAST_FORWARDED=0
    ...
    log_success ".devcontainer/features/ synced ($copied copied, $noop unchanged, $FEATURES_SYNC_DELETED removed, $FEATURES_SYNC_SKIPPED preserved, $FEATURES_SYNC_FAST_FORWARDED fast-forwarded)"
